...
public class Little {
    ...
    public Document makeDocument() {     
        ...
        WasAttributedTo attr3 = pFactory.newWasAttributedTo(null,testent.getId(),test.getId());


        WasDerivedFrom wdf = pFactory.newWasDerivedFrom(quote.getId(), original.getId());

        Document document = pFactory.newDocument();
        document.getStatementOrBundle().addAll(Arrays.asList(quote, paul, luc, attr1, attr2, attr3, original, wdf));
